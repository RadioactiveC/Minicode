...
**用 64M 参数模型、272 条数据和一台 Mac CPU，走完 SFT + RL 全流程，把工具调用准确率从 40% 推到 80%。**

Minicode 将 [MiniMind](https://github.com/jingyaogong/minimind)（64M 小语言模型）接入简化版 [ApeCode](https://github.com/ApeCodeAI/apecode) Agent Harness，探索在极低算力和有限数据条件下，如何通过 SFT → RL 两阶段训练让轻量模型习得与计算机环境交互的能力。
...
整个训练流程在单台 Mac CPU 上完成，无需 GPU。每一步设计都围绕"有限资源下最大化收益"展开：
